-- Power BI query shape 312 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            42
-- Distinct texts        7 (same query, different literals or projection)
-- Rows read             1,459,963,468
-- Rows returned         115,468,674
-- Avg duration          46,776 ms
...
WITH CTE AS (
  ...
  LEFT JOIN main.prod_homecare_actransactional_homecare.referral AS ref
    ON ref.hmcleadid = hmcl.hmcleadid
  WHERE
    NOT outcomeid IN (5, 15, 19) AND sr.createdate >= '2023-11-01'
), CTEA AS (
  SELECT
    CTEAA.hmcrequestid,
    CASE WHEN (
