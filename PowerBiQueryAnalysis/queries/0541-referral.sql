-- Power BI query shape 541 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            2
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             23,622,682
-- Rows returned         231,391
-- Avg duration          2,873 ms
...
WITH CTE AS (
  ...
  WHERE
    NOT ref.hmcleadid IS NULL
    AND (
      hmcr.createdate >= DATE_TRUNC('MONTH', ref.referredon)
      AND hmcr.createdate <= DATE_ADD(DAY, 2, DATE_TRUNC('MONTH', ref.referredon))
    )
    AND ref.billingtypeid = 3
    AND ref.orderid <> 2774
    AND hmcr.createdate >= '2025'
  ...
  WHERE
    NOT ref.hmcleadid IS NULL
    AND (
      hmcr.createdate <= LAST_DAY(ref.referredon)
      AND hmcr.createdate >= DATE_ADD(DAY, -2, LAST_DAY(ref.referredon))
    )
    AND ref.billingtypeid = 3
    AND ref.orderid <> 2774
    AND hmcr.createdate >= '2025'
