-- Power BI query shape 124 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            484
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             17,905,307,705
-- Rows returned         438,656,754
-- Avg duration          26,655 ms
...
FROM CTE1
WHERE
  Timestamp >= DATE_TRUNC('YEAR', DATE_ADD(YEAR, -1, CURRENT_TIMESTAMP()))
  AND CAST(timestamp AS DATE) <= CAST(CURRENT_TIMESTAMP() AS DATE)
