-- Power BI query shape 602 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            2
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             19,353,648
-- Rows returned         112,602
-- Avg duration          4,694 ms
...
WITH cte AS (
  ...
  SELECT
    ...
    Org,
    DMA,
    SUM(Referrals) OVER (PARTITION BY MonthYearID, org, dma ORDER BY ReferredOn) AS RunningReferrals
  FROM CTE
  GROUP BY
    referredon,
    monthyearid,
...
FROM CTE
LEFT JOIN CTEA
  ON CTEA.Referredon = cte.referredon AND ctea.org = cte.org AND ctea.dma = cte.dma
