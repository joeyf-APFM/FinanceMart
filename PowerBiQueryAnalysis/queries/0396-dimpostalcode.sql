-- Power BI query shape 396 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            6
-- Distinct texts        5 (same query, different literals or projection)
-- Rows read             98,080,350
-- Rows returned         2,664,734
-- Avg duration          49,258 ms
...
SELECT
  CTE2.*
  EXCEPT (hmcprospectid, postalcode, affiliatename, url, requeststatuscode, rn, note, affiliateid, source, campaign1, campaignid),
  CASE
    WHEN AdjChannel = 'SEM'
    AND CampaignID IN (
      21152744542,
