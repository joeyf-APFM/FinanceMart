-- Power BI query shape 441 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            4
-- Distinct texts        4 (same query, different literals or projection)
-- Rows read             3,741,962
-- Rows returned         65,108
-- Avg duration          2,721 ms
...
SELECT
  ...
  CASE
    ...
    ELSE 0
  END AS Retention,
  ROW_NUMBER() OVER (PARTITION BY CONCAT(providerid, '-', orderid) ORDER BY lastactivedate DESC) AS rn
FROM CTE
WHERE
  lastday > '2025-07-01'
