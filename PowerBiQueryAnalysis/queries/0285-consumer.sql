-- Power BI query shape 285 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            80
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             3,622,128,303
-- Rows returned         423,531,555
-- Avg duration          36,931 ms
-- Power BI datasets     728529db-9b31-4ce3-9fd4-4fdf844d7334
-- Tables                grace.grace_prod_public.consumer, grace.grace_prod_public.lead, grace.grace_prod_public.referral, grace.grace_prod_public.referral_cart_map, grace.posthog_prod.events, prod.dim_funnel_channel, prod.funnel_lead, prod_homecare_actransactional_geo.postalcode_01072025, prod_homecare_actransactional_geo.timezone, prod_homecare_actransactional_homecare.referral, prod_homecare_insite_directory.affiliate, prod_homecare_insite_directory.hmclead, prod_homecare_insite_directory.hmcrequest, prod_homecare_insite_directory.hmcscreeningresult
--
...
WITH CTE AS (
  ...
  LEFT JOIN prod_homecare_actransactional_geo.postalcode_01072025 AS tz
    ON tz.code = TRIM(hmcr.postalcode)
  LEFT JOIN prod_homecare_actransactional_geo.timezone AS t
    ON t.timezoneid = tz.timezoneid
  LEFT JOIN (
    SELECT
      hmcrequestid,
      hmcprospectid,
  ...
  SELECT
    ...
    CASE
      WHEN AdjChannel <> 'SEM'
      THEN NULL
      WHEN RequestDateTime >= '2026'
      THEN REGEXP_EXTRACT(SUBSTRING_INDEX(url, 'campaignid=', -2), '([0-9]+)', 0)
      WHEN LOCATE('adgroup', url) = 0
      THEN NULL
      ELSE RIGHT(
        LEFT(url, LOCATE('adgroup', url) - 2),
        LENGTH(LEFT(url, LOCATE('adgroup', url) - 2)) - LOCATE('campaignid=', LEFT(url, LOCATE('adgroup', url) - 2)) - 10
      )
    END AS Campaign1,
    Lead,
    LSTF,
