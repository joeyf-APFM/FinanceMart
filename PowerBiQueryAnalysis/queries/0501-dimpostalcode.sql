-- Power BI query shape 501 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            3
-- Distinct texts        3 (same query, different literals or projection)
-- Rows read             96,874,481
-- Rows returned         524,565
-- Avg duration          7,871 ms
...
WITH CTE AS (
  ...
  LEFT JOIN (
    ...
    WHERE
      ...
      AND sr.createdate >= '2023-11-01'
      AND DATE_FORMAT(ref.referredon, 'MM-dd-yyyy HH:mm:ss') >= DATE_FORMAT(sr.createdate, 'MM-dd-yyyy HH:mm:ss')
  ) AS sr
  ...
  WHERE
    NOT outcomeid IN (5, 15, 19)
    AND sr.createdate >= '2023-11-01' /* and date_format(ref.referredon, "MM-dd-yyyy HH:mm:ss") >= date_format(sr.createdate,"MM-dd-yyyy HH:mm:ss") */
), CTEA AS (
  SELECT
    CTEAA.hmcrequestid,
    CASE WHEN (
