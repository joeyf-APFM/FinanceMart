-- Power BI query shape 294 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            67
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             588,814,462
-- Rows returned         1,652,230
-- Avg duration          2,824 ms
...
SELECT
  ...
  CASE WHEN p.providerorganizationid IS NULL THEN 'Independent' ELSE po.name END AS OrgName,
  ...
  ProviderCoverage,
  DATEDIFF(DAY, firstactive, lastactive) AS DaysActive,
  CASE
    WHEN (
