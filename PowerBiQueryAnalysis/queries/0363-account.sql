-- Power BI query shape 363 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            10
-- Distinct texts        4 (same query, different literals or projection)
-- Rows read             35,990,478
-- Rows returned         15,700
-- Avg duration          2,568 ms
-- Power BI datasets     none recorded
-- Tables                community_salesforce.account, community_salesforce.opportunity, prod_homecare_acreporting_reporting.cartproviderhistory, prod_homecare_actransactional_homecare.referral, prod_homecare_actransactional_ordermanagement.order, prod_homecare_actransactional_ordermanagement.orderprovider, prod_homecare_actransactional_ordermanagement.orderstatusreasontype, prod_homecare_actransactional_ordermanagement.orderstatustype, prod_homecare_actransactional_organization.provider
--
-- Power BI's `SELECT ... FROM (...) AS `_`` envelope was stripped, so this
-- is the inner query as written, reformatted by sqlglot -- not the exact
-- bytes Power BI sent.

WITH CTE /* and a.hmc_cart_provider_id_c = 30356 */ AS (
  SELECT
  ...
  SELECT
    ...
    CTE1.OrderID,
    CONCAT(CTE1.ProviderID, '-', CTE1.OrderID) AS ProviderOrderID,
    CASE WHEN cteb.orderstatus IS NULL THEN ost.name ELSE cph.orderstatus END AS OrderStatus,
...
WHERE
  ...
  AND (
    lastactivedate > '2026-10-01' OR lastactivedate IS NULL
  )
