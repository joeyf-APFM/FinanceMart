-- Power BI query shape 493 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            3
-- Distinct texts        3 (same query, different literals or projection)
-- Rows read             80,575,461
-- Rows returned         2,898,577
-- Avg duration          8,205 ms
...
WITH CTE AS (
  ...
  LEFT JOIN (
    ...
    AND pn.rns = 1
  /* (sr.outcomeid NOT IN (5,15,19) OR sr.outcomeid is NULL) AND */
  WHERE
    hmcr.createdate >= '2024-01-01' AND sr.APFMMisattribute = 0
), CTEAA AS (
  SELECT
    *,
