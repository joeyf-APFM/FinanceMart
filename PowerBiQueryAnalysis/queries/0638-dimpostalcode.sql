-- Power BI query shape 638 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            1
-- Distinct texts        1 (same query, different literals or projection)
-- Rows read             13,167,718
-- Rows returned         2
-- Avg duration          28,454 ms
...
WITH CTE AS (
  SELECT DISTINCT
    hmcr.hmcrequestid AS Requestid,
    hmcr.hmcprospectid,
    postalcode,
  ...
  LEFT JOIN (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY hmcrequestid ORDER BY createdate DESC) AS srn,
      CASE
        WHEN userid = 1702946
        AND DATEDIFF(
          MINUTE,
  ...
  WHERE
    ...
    AND hmcr.hmcprospectid = 3540171
    AND hmcr.createdate >= '2024-01-01'
    AND sr.APFMMisattribute = 0
), CTEAA AS (
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY sr.hmcrequestid ORDER BY createdate DESC) AS srn,
    CASE
      WHEN userid = 1702946
      AND DATEDIFF(
        MINUTE,
  ...
  FROM CTE
  LEFT JOIN CTEA
    ON CTEA.hmcrequestid = CTE.Requestid
  WHERE
    rn = 1 AND APFMMisattribute = 0
  ORDER BY
    CTE.Requestid DESC
), CTE2 AS (
  SELECT
