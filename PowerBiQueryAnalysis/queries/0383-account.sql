-- Power BI query shape 383 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            7
-- Distinct texts        1 (same query, different literals or projection)
-- Rows read             49,245,847
-- Rows returned         7,000
-- Avg duration          10,382 ms
...
WITH CTE /* and a.hmc_cart_provider_id_c = 40382 */ AS (
  ...
  SELECT DISTINCT
    ...
    CASE
      ...
      THEN 500
      ELSE DATEDIFF(DAY, firstactivedate, CAST(CURRENT_TIMESTAMP() AS DATE))
    END AS ProviderOrder_Age,
    DATEDIFF(DAY, firstactivedate, firstactivation) AS DaystoActivation
  FROM CTE3
  LEFT JOIN CTED
    ON CTED.ProviderID = CTE3.ProviderID
  WHERE
    lastactivedate >= CAST(CTED.firstcasecreatedate AS DATE)
    OR lastactivedate IS NULL
), CTE5 AS (
  SELECT DISTINCT
    CTE4.ProviderID,
    CTE4.ProviderName,
