-- Power BI query shape 495 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            3
-- Distinct texts        3 (same query, different literals or projection)
-- Rows read             53,356,623
-- Rows returned         2,897,635
-- Avg duration          12,607 ms
...
WITH CTE AS (
  ...
  WHERE
    ...
    AND (
      sr.APFMMisattribute = 0 OR (
        srn = 1 AND sr.APFMMisattribute = 1
      )
    )
), CTE1 AS (
  SELECT
