-- Power BI query shape 14 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            2,959
-- Distinct texts        3 (same query, different literals or projection)
-- Rows read             1,783,677,804
-- Rows returned         127,072,981
-- Avg duration          2,610 ms
...
WITH CTE AS (
  ...
  SELECT
    ...
    calllogid,
    LagTimeStamp,
    TIMESTAMPDIFF(MINUTE, CAST(LagTimeStamp AS TIMESTAMP), Timestamp),
    CASE
...
SELECT
  Date,
  Agentname,
  COUNT(DISTINCT timestamp) AS Gaps,
  CallGapBucket
FROM CTE1
WHERE
