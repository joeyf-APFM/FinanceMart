-- Power BI query shape 349 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            17
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             2,455,891
-- Rows returned         17,272
-- Avg duration          660 ms
...
SELECT
  ...
  u.name AS CSM,
  CASE
    WHEN u.name IN ('Cindy Spainhower', 'Jessica Perdue')
    THEN 'Group'
    ELSE u.name
  END AS CSMGroup,
  c.account_id,
  c.first_activation_c AS FirstActivation,
  CASE
