-- Power BI query shape 539 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            2
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             47,987,416
-- Rows returned         2,252
-- Avg duration          5,628 ms
...
WITH CTE AS (
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'Active'
      WHEN cte1.orderstatus = 'Onboarding'
      THEN 'Onboarding'
      ELSE 'Inactive'
    END AS Status
  FROM CTE1
  ...
  SELECT
    CTE2.ProviderId,
    CTE2.OrderID,
    COUNT(DISTINCT ref.referralid) AS AfterFreeReferrals
  FROM CTE2
  LEFT JOIN main.prod_homecare_actransactional_homecare.referral AS ref
    ON ref.providerid = cte2.providerid AND ref.orderid = cte2.orderid
  WHERE
    CAST(ref.referredon AS DATE) >= lastfreelead AND NOT ref.hmcleadid IS NULL
  GROUP BY
    1,
    2
)
...
FROM CTE2
LEFT JOIN cted
  ON cted.providerid = cte2.providerid AND cted.orderid = cte2.orderid
