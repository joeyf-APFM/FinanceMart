-- Power BI query shape 418 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            5
-- Distinct texts        4 (same query, different literals or projection)
-- Rows read             81,733,625
-- Rows returned         5,000
-- Avg duration          13,202 ms
...
SELECT
  CTE2.*
  EXCEPT (hmcprospectid, postalcode, affiliatename, url, rn, source, campaign1, campaignid),
  CASE
    WHEN AdjChannel = 'SEM'
    AND CampaignID IN (
      21152744542,
