-- Power BI query shape 436 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            5
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             15,465,904
-- Rows returned         804,560
-- Avg duration          1,078 ms
...
SELECT
  *,
  ...
  CASE
    ...
    THEN 'Morning'
    WHEN HOUR(Adjust_CreateDate) IN (12, 13, 14, 15, 16)
    THEN 'Afternoon'
    WHEN HOUR(Adjust_CreateDate) IN (17, 18, 19, 20)
    THEN 'Evening'
    ELSE 'Other'
  END AS TOD,
  CASE
    ...
    ELSE 4
  END AS TODFilter,
  CAST(adjust_createdate AS DATE) AS CreateDate,
  DATE_FORMAT(
    DATE_ADD(
      MINUTE,
