-- Power BI query shape 645 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            1
-- Distinct texts        1 (same query, different literals or projection)
-- Rows read             13,947,955
-- Rows returned         1,000
-- Avg duration          25,645 ms
...
WITH CTE AS (
  ...
  WHERE
    cph.date = LAST_DAY(cph.date)
    OR cph.date = CAST(CURRENT_TIMESTAMP() AS DATE)
    AND cph.contracttype = 'CPL'
    AND (
      cph.orderstatus IN ('Active', 'Paused')
      OR cph.orderstatusreason = 'MonthlyCapReached'
    )
    AND psc.deleted = 0
    AND dma <> 'NA'
    AND date >= '2025'
