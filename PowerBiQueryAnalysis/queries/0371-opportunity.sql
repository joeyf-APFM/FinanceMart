-- Power BI query shape 371 of 663, from workspace_jf.landscape.qh.
...
SELECT DISTINCT
  CTE1.AgentName,
  CAST(g.call_start_at AS DATE) AS CreateDate,
  COUNT(
    DISTINCT CASE WHEN g.gong_process_status = 'scheduled' THEN g.conversation_id ELSE NULL END
  ) AS MTDGongCalls,
  SUM(g.duration_seconds) AS ZoomDuration
FROM CTE1
