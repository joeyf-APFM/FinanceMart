-- Power BI query shape 192 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            261
-- Distinct texts        3 (same query, different literals or projection)
-- Rows read             96,741,413
-- Rows returned         16,010
-- Avg duration          2,069 ms
...
WITH CTE AS (
  ...
  SELECT DISTINCT
    ...
    CASE
      ...
      THEN 25
      WHEN cph.monthlycap >= 100 AND cph.monthlysent IS NULL
      THEN 100
      WHEN cph.monthlycap >= 100 AND cph.monthlysent >= 0.5 * cph.monthlycap
      THEN cph.monthlycap
      WHEN cph.monthlycap >= 100 AND cph.monthlysent < 0.5 * cph.monthlycap
