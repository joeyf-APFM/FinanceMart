-- Power BI query shape 262 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            114
-- Distinct texts        6 (same query, different literals or projection)
-- Rows read             3,263,466,298
-- Rows returned         190,902,468
-- Avg duration          26,950 ms
...
WITH CTE AS (
  ...
  LEFT JOIN main.prod_homecare_actransactional_homecare.referral AS ref
    ON ref.hmcleadid = hmcl.hmcleadid
  WHERE
    NOT outcomeid IN (5, 15, 19) AND sr.createdate >= '2024-04-01'
), CTEA AS (
  SELECT
    CTEAA.hmcrequestid,
    CASE WHEN (
