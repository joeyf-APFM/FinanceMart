-- Power BI query shape 605 of 663, from workspace_jf.landscape.qh.
-- Not a schema probe: this shape reads data and returns rows.
--
-- Executions            2
-- Distinct texts        2 (same query, different literals or projection)
-- Rows read             12,120,882
-- Rows returned         16,476
-- Avg duration          1,235 ms
...
WITH CTE AS (
  SELECT
    ...
    CASE
      WHEN u2.FirstName = 'Elizabeth Palmer'
      THEN 'Elizabeth Randle'
      ELSE u2.firstname
    END AS HCAM,
    u.FirstName AS CSM
...
SELECT
  CTE.*,
  CAST(DATE_TRUNC('MONTH', FirstActiveDate) AS DATE) AS FirstActiveMonth,
