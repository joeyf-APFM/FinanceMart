...
| `ar_transaction_key` | BIGINT | Yes | Derived | FK to [[fact_ar_transaction]] |
...
|---|---|---|---|
| [[fact_ar_transaction]] | `s.ar_transaction_key = t.ar_transaction_key` | N:1 | The current-state document |
| [[dim_customer]] | `s.customer_key = dc.customer_key` | N:1 | `LEFT JOIN` — no FK declared |
| [[dim_currency]] | `s.currency_key = c.currency_key` | N:1 | |
| [[dim_date]] | `d.full_date = s.snapshot_date` | N:1 | **No `date_key` on this table** — join on the date |
| [[mart_ar_aging]] | This snapshot is one of its two bases | — | `computation_basis = 'snapshot'` |
| [[fact_ar_apply]] | Validation, not a join | — | The reconstruction that checks this table |
...
A non-zero delta is expected to be small and explainable. A large one usually means the apply trail is incomplete across the open/history boundary — which is the dependency the design already flags, and which would invalidate every reconstructed figure in [[mart_ar_aging]].

### Reading `gp_aging_bucket` from here, not from the document

[[fact_ar_transaction]]`.gp_aging_bucket` is current state and null for history rows. **This table has the bucket as it stood on each date**, which is what T-09 needs for any period other than today.
...
- **The first `snapshot_date` bounds the table.** Questions before it must be reconstructed, and reconstructions and snapshots coexist in [[mart_ar_aging]] under `computation_basis` rather than being silently blended.
- **This is not the same risk profile as [[snap_period_close_daily]].** That one has no fallback and is tagged `irrecoverable_if_delayed`. This one has [[fact_ar_apply]].
