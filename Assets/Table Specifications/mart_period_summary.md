...
| **Sources** | [fact_gl_posting](fact_gl_posting.md) via [dim_gl_account](dim_gl_account.md); close state from [snap_period_close_daily](snap_period_close_daily.md) |
...
1. **It rolls up on `account_category_number`**, not the raw account. Chosen so this table is a *summary* rather than a second copy of [mart_account_period_activity](mart_account_period_activity.md).
2. **It carries as-of close state.** Read from [snap_period_close_daily](snap_period_close_daily.md), **not from `SY40100` directly** — which is the whole reason the snapshot exists. `SY40100` answers *"is it closed now"*; every vintage measure needs *"was it closed as of the reporting date."*
...
| `posting_type` | INT | Yes | [dim_gl_account](dim_gl_account.md) | Balance-sheet vs P&L |
...
| `currency_key` | BIGINT | Yes | Fact | **PK.** FK to [dim_currency](dim_currency.md) |
...
| [dim_legal_entity](dim_legal_entity.md) | `m.legal_entity_code = le.legal_entity_code` | N:1 | |
| [snap_period_close_daily](snap_period_close_daily.md) | Already resolved onto the mart | — | **Do not re-join it.** The two close columns are the resolved answer |
| [dim_gl_account](dim_gl_account.md) | `m.account_category_number = a.account_category_number` | **1:N** | **Not an account-grain join.** See below |
| [mart_account_period_activity](mart_account_period_activity.md) | `(legal_entity_code, fiscal_year, fiscal_period, currency_key)` | 1:N | Only as a drill-down, and only after pinning the other mart's flags |
...
- **Only series 2 and 3 are carried.** Purchasing, Inventory, Payroll and Project close state is in [snap_period_close_daily](snap_period_close_daily.md) and not here — if a consumer needs those, the mart needs new columns rather than a join.
