...
| `customer_key` | BIGINT | Yes | Derived on `fin_customer_id` | FK to [dim_customer](dim_customer.md). **NULL IS MEANINGFUL AND MUST NOT BE FILTERED AWAY** — see below |
...
| `family_file_id` | STRING | Yes | Via [bridge_customer_to_family](bridge_customer_to_family.md) | **100.00% for FY2023-2026, roughly 1.8% before mid-2022.** PROFILE |
| `business_unit_id` | STRING | Yes | PROFILE | The community. **Confirm whether `ipr` carries it directly or it must come through [bridge_customer_to_business_unit](bridge_customer_to_business_unit.md) as of the charge date** |
...
| `date_key` | INT | Yes | Derived on `charge_date` | FK to [dim_date](dim_date.md) |
| `charge_type` | STRING | Yes | **PROFILE** | The billing stream. **[mart_billing_by_stream](mart_billing_by_stream.md) aggregates on it, so its domain must be enumerated *before* that mart is built rather than after** (T-11) |
...
| `currency_key` | BIGINT | Yes | Derived | FK to [dim_currency](dim_currency.md). **PROFILE whether `ipr` records a currency at all**; if not, default to the entity's functional currency **and say so here rather than assuming USD silently** |
| `gp_invoice_number` | STRING | Yes | Via `ipr_invoice.gp_invoice_num` | The GP invoice this charge was billed on. **Null until invoiced** |
| `invoice_line_key` | BIGINT | Yes | Derived | FK to [fact_invoice_line](fact_invoice_line.md) where matched. Null where not — see that table's coverage figures |
...
|---|---|---|---|
| [dim_customer](dim_customer.md) | `f.customer_key = dc.customer_key` | N:1 | Declared FK. **`LEFT JOIN` — see below** |
| [fact_invoice_line](fact_invoice_line.md) | `f.invoice_line_key = il.invoice_line_key` | N:1 | `LEFT JOIN`. Only matched charges |
| [dim_date](dim_date.md) | `f.date_key = d.date_key` | N:1 | On `charge_date`, once T-10 confirms which column that is |
| [dim_currency](dim_currency.md) | `f.currency_key = c.currency_key` | N:1 | May be entirely derived rather than sourced |
| [bridge_customer_to_family](bridge_customer_to_family.md) | `f.customer_key = b.customer_key` | **1:N** | `family_file_id` may already be resolved — **do not resolve twice** |
| [bridge_customer_to_business_unit](bridge_customer_to_business_unit.md) | as-of `charge_date` | **1:N** | Fan-out **and** as-of. Only if `ipr` does not carry it directly |
| [mart_billing_by_stream](mart_billing_by_stream.md) | This fact is its source | — | Aggregate |
| [fact_gl_posting](fact_gl_posting.md) | — | — | **No join, and no reconciliation.** Different measure classes |
...
A populated `move_in_date` with no `customer_key` is a resident who moved in and has not been billed. **This is a use case, not a defect**, and it is available the moment the fact exists. `unbilled_charge_amount` on [mart_billing_by_stream](mart_billing_by_stream.md) is this figure materialised.
...
- **Do not build [mart_billing_by_stream](mart_billing_by_stream.md) before T-11.** *"An unenumerated stream column becomes an ever-growing pivot no one can validate."*
...
- **`business_unit_id` may need the as-of bridge.** If it does, the resolution belongs in the pipeline as it does on [mart_writeoff](mart_writeoff.md), not in every consumer's query.
